...
for length in [56]:#range(8,65,8):
  print(length)
  sizes.append(length)

  problem = mlrose.DiscreteOpt(length=length, fitness_fn=fitness, maximize=True, max_val=2)
  init_state = np.random.randint(length,size=(length))#([0, 1, 2, 3, 4, 5, 6, 7])
  # SA
  t1 = time.time()
  ...
  # MIMIC
  t4 = time.time()
  best_state,best_fitness_mimic,curve_mimic = mlrose.mimic(problem, pop_size=200, keep_pct=0.3, max_attempts=100,
          max_iters=200, curve=True, random_state=1)
...
plt.title("Queens: Fitness of problem sizes with different algorithms")
plt.show()
